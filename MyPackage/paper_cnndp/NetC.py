...
class Module_Conv(torch.nn.Module):
    def __init__(self, in_N: int = 1, depth: int = 2, width: int = 2, out_N: int = 1,
        ...
        super(Module_Conv, self).__init__()
        
        self.NN_Conv = DNN(in_N, depth, width, out_N,
    ...
    def setup_optimizer(self, initial_lr: float = 1e-3):
        ...
        dict1 = {'params': self.NN_Conv.parameters(), 'lr': initial_lr}
    ...
    def fit(self, epochs, lr_halflife, filename, save: bool = True, save_every: int = 1, reg_lambda: float = 0,
            initial_lr: float = 1e-3, loss_fn: str = 'mse', print_every: int = 10, valid_measure: str = 'mse'):
        ################## model info ##################
        vars = locals()
        if 'self' in vars.keys(): del vars['self']
        if '__class__' in vars.keys(): del vars['__class__']
